...
test('parses only an approved Thailand city and bounded private-review page', () => {
  ...
  assert.deepEqual(parsePreviewArgs(['--city', 'Khao Sok', '--count', '3']), {
    ok: true,
    input: { cityKey: 'khao-sok', count: 3, start: 1 },
  })
  assert.deepEqual(parsePreviewArgs(['--city', 'Mae Hong Son', '--count', '5']), {
    ok: true,
    input: { cityKey: 'mae-hong-son', count: 5, start: 1 },
  })
