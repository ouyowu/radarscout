...
function SummaryItem({ label, value }: SummaryItemProps) {
  return (
    <div className="border border-[#e8dfd2] bg-white p-3 sm:p-4">
      <dt className="text-xs font-black uppercase tracking-[0.12em] text-[#6b5d4d]">{label}</dt>
      <dd className="mt-1.5 text-sm font-black leading-6 text-[#101820] sm:mt-2">{formatValue(value)}</dd>
    </div>
  )
}
...
function SummaryList({ label, values }: SummaryListProps) {
  return (
    <div className="border border-[#e8dfd2] bg-white p-3 sm:p-4">
      <dt className="text-xs font-black uppercase tracking-[0.12em] text-[#6b5d4d]">{label}</dt>
      <dd className="mt-1.5 text-sm font-black leading-6 text-[#101820] sm:mt-2">{formatList(values)}</dd>
    </div>
  )
}
...
export function TripIntentSummary({
  ...
  return (
    <dl data-testid="ai-trip-intent-summary" className="mt-4 grid gap-2 sm:mt-5 sm:gap-3 md:grid-cols-2 xl:grid-cols-3">
