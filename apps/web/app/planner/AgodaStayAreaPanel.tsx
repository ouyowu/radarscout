...
import type {
  AgodaStayDecisionContext,
  ReviewedAgodaStayAreaOffer,
} from '@/lib/affiliates/agodaStayAreaOffers'
import { buildAgodaStayAreaDecisions } from '@/lib/affiliates/agodaStayAreaOffers'
...
type AgodaStayAreaPanelProps = {
  ...
  decisionContext: AgodaStayDecisionContext
}

export function AgodaStayAreaPanel({ destination, offers, tripContext, decisionContext }: AgodaStayAreaPanelProps) {
  const decisions = buildAgodaStayAreaDecisions(offers, destination, decisionContext)
  if (decisions.length === 0) return null
  const hasConfirmedOccupancy = tripContext.adultCount !== null && tripContext.childCount !== null

  return (
    <section
      aria-label={`Reviewed places to stay in ${decisions[0].area.city}`}
    ...
    >
      ...
      <div className="mt-2 max-w-3xl">
        <h3 className="font-rs-display text-2xl font-semibold tracking-[-0.025em] text-rs-ink sm:text-3xl">
          Choose a reviewed {decisions[0].area.city} base
        </h3>
      ...
      <div className="mt-4 rounded-rs-md border border-rs-sage-200 bg-rs-sage-100/60 px-4 py-3">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-rs-terracotta-600">Budget reality check</p>
        <p className="mt-1 text-sm font-semibold leading-6 text-rs-muted">{decisions[0].budgetGuidance}</p>
      </div>

      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        {decisions.map(({ area, offer, fitReasons }) => (
          <article key={area.id} className="flex h-full flex-col rounded-rs-md border border-rs-sage-200 bg-rs-sand-50 p-5">
            ...
            <div className="mt-4 border-t border-rs-sage-200 pt-4">
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-rs-terracotta-600">Why this fits your trip</p>
              <ul className="mt-2 space-y-2 text-sm font-semibold leading-5 text-rs-muted">
                {fitReasons.map(reason => (
                  <li key={reason} className="flex gap-2">
                    <span aria-hidden="true" className="text-rs-forest-500">•</span>
                    <span>{reason}</span>
                  </li>
                ))}
              </ul>
