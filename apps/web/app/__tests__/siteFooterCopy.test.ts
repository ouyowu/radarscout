...
describe('SiteFooter public copy safety', () => {
  it('uses RadarScout-owned supplier partnership mailto wording', () => {
    ...
    expect(publicSiteContentSource).not.toMatch(/Bókun Supplier Partnership Inquiry/i)
  })

  it('keeps footer positioning aligned with guided discovery boundaries', () => {
    expect(siteFooterSource).toContain('Personalized Thailand day-trip discovery with safe booking partner handoff.')
    expect(siteFooterSource).not.toContain('AI Planner')
