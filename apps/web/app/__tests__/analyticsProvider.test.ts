...
const funnelEventSource = readFileSync(
  new URL('../../lib/analytics/funnelEventPayload.ts', import.meta.url),
  'utf8',
)
...
describe('Vercel analytics provider', () => {
  ...
  it('flushes approved funnel events through the Vercel analytics provider without secrets', () => {
    ...
    expect(funnelEventSource).toContain('booking_partner_handoff_clicked')
